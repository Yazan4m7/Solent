...
//            $devices = device::all();
          ...
          // dd($devices);
           ...
           $inactiveSinteringCases = sCase::whereHas('jobs', function ($q) {
            ...
            // Load employees for each stage assignment
            $designEmployees = User::whereHas('permissions', function ($q) {
                $q->where('permission_id', 1);
            })->get();

            $millingEmployees = User::whereHas('permissions', function ($q) {
                $q->where('permission_id', 2);
            })->get();

            $printingEmployees = User::whereHas('permissions', function ($q) {
                $q->where('permission_id', 3);
            })->get();

            $sinteringEmployees = User::whereHas('permissions', function ($q) {
                $q->where('permission_id', 4);
            })->get();

            $pressingEmployees = User::whereHas('permissions', function ($q) {
                $q->where('permission_id', 5);
            })->get();

            $finishingEmployees = User::whereHas('permissions', function ($q) {
                $q->where('permission_id', 6);
            })->get();

            $qcEmployees = User::whereHas('permissions', function ($q) {
        ...
        return view('cases.admin-dashboardv2', compact(
            ...
            'designEmployees', 'millingEmployees', 'printingEmployees',
            'sinteringEmployees', 'pressingEmployees', 'finishingEmployees', 'qcEmployees'
        ));
    }
