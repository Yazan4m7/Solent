...
class CaseController extends Controller
{
    ...
    {
        $this->middleware('refresh.case.cache');
    }
...
//
    ...
    public function adminDashboard_v2()
    {
        ...
        if (Cache::has($cacheKey)) {
            ...
            // Define base queries for each stage with eager loading
            $baseQuery = sCase::with(['client:id,name', 'jobs' => function ($q) {
                $q->select('id', 'unit_num', 'case_id', 'stage', 'assignee', 'is_active', 'is_set', 'device_id', 'type', 'material_id', 'color', 'style');
            }]);

            // Apply different queries for admin vs normal users
            if ($isAdmin) {
                // Admin queries - use more efficient whereHas instead of collection filtering + whereIn
                // Active cases for each stage
                $aDesign = (clone $baseQuery)
                ...
                $aMilling = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 2)
                            ->where('is_set', 1); // <- Milling specific
                    })->get();

                // Fixed 3D printing active cases query - simplified to just check for jobs in stage 3
                $aPrinting = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 3)
                            ->where(function ($query) {
                                $query->where('is_set', 1)
                                    ->orWhere('is_active', 1)
                                    ->orWhereNotNull('printing_build_id');
                            });
                    })->get();

                $aSintering = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 4)
                            ->where('is_set', 1);
                    })->get();

                $aPressing = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 5)
                            ->where('is_set', 1);
                    })->get();

                $aFinishing = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 6)
                            ->whereNotNull('assignee')
                            ->where('is_active', 1);
                ...
                $aDelivery = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 8)
                            ->whereNotNull('delivery_accepted'); // < Assigned cases stays in waiting until employee accepts
                    })->get();
            } else {
                // User-specific queries
                $aDesign = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) use ($currentUserId) {
                        $q->where('stage', 1)->where('assignee', $currentUserId)->where('is_active', 1);;
                    })->get();

                $aMilling = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 2)->where('is_set', 1);
                    })->get();

                $aPrinting = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 3)->where('is_set', 1);
                    })->get();

                $aSintering = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 4)->where('is_set', 1);
                    })->get();

                $aPressing = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 5)->where('is_set', 1);
                    })->get();

                $aFinishing = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) use ($currentUserId) {
                        $q->where('stage', 6)->where('is_active', 1);
                    })->get();

                $aQC = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) use ($currentUserId) {
                        $q->where('stage', 7)->where('is_active', 1);
                    })->get();

                $aDelivery = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) use ($currentUserId) {
                        $q->where('stage', 8)->where('assignee', $currentUserId)->whereNotNull('delivery_accepted');
                    })->get();
            }
            // Waiting cases
            $wDesign = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 1)->whereNull('assignee');
                })->get();

            $wMilling = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 2)
                        ->where(function ($query) {
                            $query->whereNull('is_set')
                                ->orWhere('is_set', 0);
                        });
                })->get();

            $wPrinting = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 3)
                        ->where(function ($query) {
                            $query->whereNull('is_set')
            ...
            $wSintering = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 4)->whereNull("is_active");
                })->get();

            $wPressing = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 5)->whereNull("is_active");
                })->get();

            $wFinishing = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 6)->whereNull('assignee');
                    //->whereNull("is_active")->where('is_set', 0);
                })->get();

            $wQC = (clone $baseQuery)
                ->whereHas('jobs', function ($q) {
                    $q->where('stage', 7)->whereNull('assignee');
                    //->whereNull("is_active")->where('is_set', 0);
                })->get();

            // Delivery for admin/employee
            if ($isAdmin || $permissions->contains('permission_id', 129)) {
                $wDelivery = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) {
                        $q->where('stage', 8)->whereNull('delivery_accepted');
                    })->get();
            } else {
                $wDelivery = (clone $baseQuery)
                    ->whereHas('jobs', function ($q) use ($currentUserId) {
                        $q->where('stage', 8)->where('assignee', $currentUserId)->whereNull('is_set');
                    })->get();
            }

            // Original device queries - inefficient with multiple queries per device:
            /*
            $devices = device::all();
            foreach ($devices as $device) {
                $device->jobsWaiting = job::where('device_id', $device->id)->where('is_set', 0)->count();
                $device->jobsSet = job::where('device_id', $device->id)->where('is_set', 1)->count();
                $device->jobsActive = job::where('device_id', $device->id)->where('is_active', 1)->count();
            }
            */
        }

            // More efficient device queries - use a single query with grouping:
//            $devices = device::all();
            ...
            //dd($deviceStats);
            //dd($deviceStats);
            //dd($deviceStats ,$devices);
            foreach ($devices as $device) {
                foreach ([ 2, 3, 4, 5,] as $stage) {
                    Log::info('[CaseController] waiting/active counts: '.$device->countOfUnits($stage, false). '/'. $device->countOfUnits($stage, true));
                    if($stage == 4) {
                        $deviceUnitsCounts[$device->id][$stage]['waiting'] =$device->sinteringWaitingCases();
                        $deviceUnitsCounts[$device->id][$stage]['active'] =  $device->sinteringActiveCases();
                    } else {
                        $deviceUnitsCounts[$device->id][$stage]['waiting'] = $device->countOfUnits($stage, false);
                        $deviceUnitsCounts[$device->id][$stage]['active'] = $device->countOfUnits($stage, true);
                        //stage : 3 , Device : 44 , waiting

                    }
                    }

                $deviceUnitsCounts[$device->id]['waitingBuilds'] = Build::where('device_used', $device->id)
                    ->whereNotNull('set_at')->whereNull('finished_at')->whereNull('started_at')->count() ;

          $deviceUnitsCounts[$device->id]['activeBuilds'] =  Build::where('device_used', $device->id)
...
//                $stats = $deviceStats->get($device->id);
          ...
          // dd($devices);
            ...
            $sinteringCases = sCase::whereHas('jobs', function ($q) {
                $q->where('stage', 4);

            })->get();

           $inactiveSinteringCases = sCase::whereHas('jobs', function ($q) {
                $q->where('sintering_build_id', 3)
                    ->where('stage', 4);
            })->get();
            $labs = lab::all();
...
//dd($deviceUnitsCounts);
        return view('cases.admin-dashboardv2', compact(
            'labs', 'wDesign', 'aDesign',
            'wMilling', 'aMilling', 'wPrinting', 'aPrinting',
            'wSintering', 'aSintering', 'wPressing', 'aPressing',
            'wFinishing', 'aFinishing', 'wQC', 'aQC', 'wDelivery',
            'aDelivery', 'drivers', 'activeOuterTab', 'devices',
            'permissions','sinteringCases','inactiveSinteringCases'
        ));
    }
    ...
    public function assignToMe($caseId, $stage, $returnMessages = true)
    {
        ...
        foreach ($jobs as $job) {
            ...
            // Save device_id if provided for specific stages
            if ($deviceId && in_array($stage, [2, 3, 4, 5])) { // milling, 3dprinting, sintering, pressing
        ...
        // Add device_id to log if provided for specific stages
        if ($deviceId && in_array($stage, [2, 3, 4, 5])) {
            $logData['device_id'] = $deviceId;
        }

        $log = new caseLog($logData);
        $log->save();
        if ($returnMessages)
